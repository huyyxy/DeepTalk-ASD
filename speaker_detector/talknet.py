...
class TalkNetSpeakerDetector:
    ...
    def evaluate(self):
        ...
        for track_id, face_frames in list(self.video_buffer.items()):
            ...
            #     audio_embed = self.model.forward_audio_frontend(audio_features)
            #     visual_embed = self.model.forward_visual_frontend(video_features)
            ...
            #     # Get prediction
            #     output = self.model.forward_audio_visual_backend(audio_embed, visual_embed)
            #     score = F.softmax(output, dim=1)[0, 1].item()  # Probability of speaking
...
# For direct usage without the interface
if __name__ == "__main__":
    # Example usage
    detector = TalkNetSpeakerDetector(device='cuda')
    
    # Simulate video input (in practice, you'd get this from your face tracker)
    # detector.append_video([{'id': 1, 'image': np.zeros((100, 100, 3), dtype=np.uint8)}])
    # 
    # # Simulate audio input (16kHz, 16-bit, mono)
    # detector.append_audio(np.zeros(16000, dtype=np.int16))
    # 
    # # Get predictions
    # results = detector.evaluate()
    # print("Active speaker results:", results)
    pass
